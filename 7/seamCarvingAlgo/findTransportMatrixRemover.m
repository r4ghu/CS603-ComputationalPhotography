function [T, transBitMask] = findTransportMatrixRemover(sizeReduction, image)
    ...
    imageNoRow = image;
    for i = 2 : size(T, 1)
        energy = energyRGBRemove(imageNoRow);
    ...
    imageNoColumn = image;
    for j = 2 : size(T, 2)
        energy = energyRGBRemove(imageNoColumn);
    ...
    for i = 2 : size(T, 1)
        ...
        for j = 2 : size(T, 2)
            energy = energyRGBRemove(imageWithoutRow);
